...
class Algorithm(BaseSLTFModel):
    ...
    def train(self):
        ...
        writer = tf.summary.FileWriter("./checkpoints/SL/NaiveLSTM/stock/")
        writer.add_graph(self.session.graph)
        for step in range(self.train_steps):
            batch_x, batch_y = self.env.get_batch_data(self.batch_size)
            acc, loss = self.session.run([self.train_op, self.loss], feed_dict={self.x: batch_x, self.label: batch_y})
            if (step + 1) % 1000 == 0:
